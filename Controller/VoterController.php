...
class VoterController extends VoteAppController
{
    ...
    private function processRewards($config, $user)
    { // Donne les récompenses au user passé selon la configuration donnée
    ...
    private function executeServerReward($config, $user, $reward)
    { // execute la commande d'une récompense si les serveurs de la config sont ouverts
        ...
        if (!in_array(false, $servers_online)) { // si tous les serveurs sont allumés
            ...
            if (empty($config['servers'])) { // si on a pas de liste, on prend celui par défaut
                ...
                if ($reward['need_connect_on_server'] == "true") {
                    $call = $this->Server->call(array('IS_CONNECTED' => array($user['pseudo'])));
                ...
                $continue = false;
                if ($reward['need_connect_on_server'] == "true") {
                    foreach ($config['servers'] as $serverId) { // on parcours les serveurs

                        $call = $this->Server->call(array('IS_CONNECTED' => array($user['pseudo'])), $serverId);
                        if ($call['IS_CONNECTED']) {
                            $continue = true;
                            break;
                        }
                ...
                if ($continue) {
                    foreach ($config['servers'] as $serverId) {
                        $this->Server->commands($cmd, $serverId); // on envoie la commande puis enregistre le vote
                    }
                } else {
                    return 'VOTE__ERROR_NEED_CONNECT_ON_SERVER';
                }
